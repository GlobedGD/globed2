...
struct GLOBED_MODIFY_ATTR HookedLoadingLayer : Modify<HookedLoadingLayer, LoadingLayer> {
    ...
    $override void loadAssets()
    {
        ...
        PreloadManager::get().enterContext(m_fromRefresh ? PreloadContext::Reloading : PreloadContext::Loading);
    ...
    void advanceLoad(float)
    {
        ...
        this->setLabelText(
            fmt::format("Globed: preloading assets.. ({}/{})", pm.getLoadedCount(), pm.getTotalCount()).c_str());
    }
