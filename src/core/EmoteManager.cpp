...
void EmoteManager::registerEmote(uint32_t id, std::string name)
{
    ...
    auto pos = std::lower_bound(m_sortedEmoteIds.begin(), m_sortedEmoteIds.end(), id);
...
std::shared_ptr<Sound> EmoteManager::playEmoteSfx(uint32_t id, std::shared_ptr<RemotePlayer> player)
{
    ...
    auto res =
        player ? PlayerSound::create(it->second.c_str(), player).map([](auto s) -> std::shared_ptr<Sound> { return s; })
...
$on_game(Loaded)
{
    ...
    std::initializer_list<std::pair<uint32_t, uint32_t>> ranges = {
        {0, 100}, {200, 100}, {300, 100}, {400, 100}, {500, 100}, {600, 100}, {700, 100},
    };
