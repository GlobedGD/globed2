...
static Duration getPingInterval(uint32_t sentPings)
{
    switch (sentPings) {
    case 0:
        return Duration::fromSecs(1);
    case 1:
        return Duration::fromSecs(2);
    case 2:
        return Duration::fromSecs(3);
    case 3:
        return Duration::fromSecs(5);
    case 4:
        return Duration::fromSecs(10);
    default:
        return Duration::fromSecs(300); // rare, because we already know the average latency
    }
}
...
static int rankError(const qsox::SocketAddress &addr, qn::ConnectionType type, const qn::ConnectionError &err)
{
    ...
    if (err.isTransportError()) {
        ...
        if (std::holds_alternative<qn::TransportError::HandshakeFailure>(te.m_kind)) {
            ...
            if (code == Code::DefragmentationError || code == Code::TooUnreliable ||
                code == Code::InvalidQunetDatabase) {
                score += 100'000'000;
            }
        }
    }
...
template <typename T, typename P> struct GeodeTaskAwaiter : public arc::PollableBase<GeodeTaskAwaiter<T, P>, T> {
...
static void updateServers(ConnectionInfo &info, auto &newServers)
{
    ...
    for (auto srv : newServers) {
        GameServer gameServer{.id = srv.getId(),
                              .stringId = srv.getStringId(),
                              .url = srv.getAddress(),
                              .name = srv.getName(),
                              .region = srv.getRegion()};

        log::debug("Added game server: {} (ID: {}, URL: {}, Region: {})", gameServer.name,
                   static_cast<int>(gameServer.id), gameServer.url, gameServer.region);
...
NetworkManagerImpl::NetworkManagerImpl() : m_runtime(Runtime::create(2)), m_workerState(createWorkerState())
{
    ...
    m_runtime->setTerminateHandler([](const std::exception &e) {
        utils::terminate(fmt::format("arc runtime terminated due to unhandled exception: {}", e.what()));
    });

    m_runtime->spawn([](auto *self) -> arc::Future<> { co_await self->asyncInit(); }(this));
}
...
void NetworkManagerImpl::shutdown()
{
    ...
    arc::Notify notify;
    m_centralConn->setConnectionStateCallback([&](qn::ConnectionState state) { notify.notifyOne(); });
    m_gameConn->setConnectionStateCallback([&](qn::ConnectionState state) { notify.notifyOne(); });

    m_centralConn->disconnect();
    m_gameConn->disconnect();

    // allow some short cleanup
    (void)m_runtime->blockOn(arc::timeout(Duration::fromMillis(100), [&](this auto self) -> arc::Future<> {
        while (true) {
...
Future<> NetworkManagerImpl::asyncInit()
{
    ...
    if (globed::value<bool>("net.dont-override-dns").value_or(false)) {
        ...
        qn::Resolver::get().setCustomDnsServers(qsox::Ipv4Address{1, 1, 1, 1}, qsox::Ipv4Address{8, 8, 8, 8});
    }
    ...
    m_centralConn->setConnectionStateCallback(
        [this](qn::ConnectionState state) { this->onCentralStateChanged(state); });
    ...
    m_gameConn->setConnectionStateCallback([this](qn::ConnectionState state) { this->onGameStateChanged(state); });
...
Future<> NetworkManagerImpl::threadWorkerLoop()
{
    ...
    case Connected: {
        ...
        co_await arc::select(
            ...
            arc::selectee(m_workerState.pingResultRx.recv(),
                          [&](auto result) -> arc::Future<> {
                              ...
                              if (it != servers.end()) {
                                  ...
                                  log::debug("Ping to server {} arrived, latency: {}ms avg, {}ms last, stable: {}",
                                             srvkey, it->second.avgLatency, it->second.lastLatency,
                                             unstable ? "no" : "yes");
                                  ...
                                  if (unstable) {
                                      ...
                                      m_workerState.nextGSPing =
                                          std::min(Instant::now() + Duration::fromSecs(5), m_workerState.nextGSPing);
                                  }
                              }
                          }),

            arc::selectee(arc::sleepUntil(m_workerState.nextGSPing), [&] {
                auto info = this->connInfo();
...
Future<> NetworkManagerImpl::threadGameWorkerLoop()
{
    ...
    co_await arc::select(arc::selectee(m_gameWorkerNotify.notified()),

                         // only poll for new join requests if there is no current request
                         arc::selectee(
                             m_gameWorkerState.joinRx->recv(),
                             [&](auto result) {
                                 if (!result)
                                     return;
    ...
    if (!showPopup) {
        ...
        if (!abortCause->first.empty()) {
            ...
            if (err == qn::ConnectionError::Success) {
                ...
                message = fmt::format("Error connecting to <cg>{} ({})</c>: <cy>{}</c>.", addr.toString(),
                                      connTypeToString(type), cause.message());
            }
        }
    ...
    FunctionQueue::get().queue([reconnecting, showPopup, message = std::move(message)] {
        ...
        if (showPopup) {
            if (reconnecting) {
                ...
                auto alert = PopupManager::get().alert("Globed Error", message);
                alert.showQueue();
            }
        }
    });
...
void NetworkManagerImpl::threadPingGameServers(LockedConnInfo &info)
{
    ...
    for (auto &[srvkey, server] : info->m_gameServers) {
        ...
        if (elapsed >= interval) {
            ...
            if (!res) {
                log::warn("Failed to ping game server {} ({}, id {}): {}", server.url, server.name, (int)server.id,
                          res.unwrapErr());
            }
    ...
    log::debug("Scheduling next game server ping in {:.3f}s", (nextPing.durationSince(now)).seconds<float>());
...
Result<> NetworkManagerImpl::sendMessageToConnection(qn::Connection &conn, std::optional<ConnectionLogger> &logger,
...
void NetworkManagerImpl::sendToGame(std23::function_ref<void(GameMessage::Builder &)> &&func, bool reliable,
                                    bool uncompressed)
{
    capnp::MallocMessageBuilder msg;
...
Result<> NetworkManagerImpl::connectCentral(std::string_view url)
{
    ...
    if (!res) {
        FunctionQueue::get().queue([url, err = std::move(res).unwrapErr()] mutable {
            globed::alertFormat("Globed Error", "Failed to connect to central server at <cp>'{}'</c>: <cy>{}</c>", url,
                                err.message());
        });
...
void NetworkManagerImpl::dumpNetworkStats()
{
    ...
    auto describe = [](const qn::StatWholeSnapshot &snap) {
        ...
        log::info("> Compression savings: {} bytes ({}%), avg ratio {} (sent); {} bytes ({}%), avg ratio {} (recv)",
                  snap.compUpSavedBytes, snap.compUpSavedPercent, snap.compUpAvgRatio, snap.compDownSavedBytes,
                  snap.compDownSavedPercent, snap.compDownAvgRatio);
        ...
        if (snap.upReliable || snap.downReliable) {
            ...
            log::info("> Ack delay: min {}, max {}, avg {}", snap.minAckDelay.toString(), snap.maxAckDelay.toString(),
                      snap.avgAckDelay.toString());
        }
    };
...
std::optional<uint8_t> NetworkManagerImpl::getPreferredServer()
{
    ...
    if (!servers.empty()) {
        auto it = std::min_element(servers.begin(), servers.end(), [](const auto &a, const auto &b) {
...
{
    auto info = this->connInfo();
    if (!info || info->m_centralUrl.empty())
        return "";
...
{
    if (!m_centralLogger && !m_gameLogger)
        return;
...
void NetworkManagerImpl::addListener(const std::type_info &ty, void *listener, void *dtor)
{
    ...
    log::debug("Added listener {} for message type '{}', priority: {}", listener, ty.name(),
...
void NetworkManagerImpl::removeListener(const std::type_info &ty, void *listener)
{
    ...
    log::debug("Removing listener {} for message type '{}'", listener, ty.name());
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.setCheckRoomState(); });
}
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.initRequestLevelList(); });
}
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.setLeaveRoom(); });
}
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.initGetTeamMembers(); });
}
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.initFetchCredits(); });
}

void NetworkManagerImpl::sendGetDiscordLinkState()
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.initGetDiscordLinkState(); });
}
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.setGetFeaturedLevel(); });
}

void NetworkManagerImpl::sendSendFeaturedLevel(int32_t levelId, const std::string &levelName, int32_t authorId,
                                               const std::string &authorName, uint8_t rateTier, const std::string &note,
                                               bool queue)
{
    this->sendToCentral([&](CentralMessage::Builder &msg) {
...
void NetworkManagerImpl::sendAdminNotice(const std::string &message, const std::string &user, int roomId, int levelId,
                                         bool canReply, bool showSender)
{
    this->sendToCentral([&](CentralMessage::Builder &msg) {
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { auto fetchUser = msg.initAdminFetchMods(); });
}
...
void NetworkManagerImpl::sendAdminUpdateUser(int32_t accountId, const std::string &username, int16_t cube,
                                             uint16_t color1, uint16_t color2, uint16_t glowColor)
{
    this->sendToCentral([&](CentralMessage::Builder &msg) {
...
void NetworkManagerImpl::sendPlayerState(const PlayerState &state, const std::vector<int> &dataRequests,
                                         CCPoint cameraCenter, float cameraRadius)
{
    auto info = this->connInfo();
...
{
    this->sendToGame([&](GameMessage::Builder &msg) { data::encode(scripts, msg.initSendLevelScript()); }, true, true);
}
...
void NetworkManagerImpl::joinSessionWith(LockedConnInfo &info, std::string_view serverUrl, SessionId id,
                                         bool platformer, bool editorCollab)
{
...
{
    this->sendToCentral([&](CentralMessage::Builder &msg) { msg.initLeaveSession(); });

    this->sendToGame([&](GameMessage::Builder &msg) { msg.initLeaveSession(); });
}
...
Result<> NetworkManagerImpl::onCentralDataReceived(CentralMessage::Reader &msg)
{
    ...
    case CentralMessage::ADMIN_FETCH_MODS_RESPONSE: {
        this->invokeListeners(
            data::decodeUnchecked<msg::AdminFetchModsResponseMessage>(msg.getAdminFetchModsResponse()));
    } break;
