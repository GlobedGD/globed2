...
Future<> ConnectionLogger::setup(std::filesystem::path path)
{
    ...
    m_handle = arc::spawn([](ConnectionLogger *self, auto pRx, auto tRx) -> Future<> {
        while (true) {
            co_await arc::select(arc::selectee(self->m_reset.notified(),
                                               [&] {
                                 ...
                                 arc::selectee(tRx.recv(), [&](auto result) {
                                     if (!result)
                                         return;
