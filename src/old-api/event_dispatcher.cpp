...
template <typename C, typename Ret, typename... Args> struct function_traits<Ret (C::*)(Args...) const> {
    using return_type = Ret;
    using arg_types = std::tuple<Args...>;
};

template <template <typename, typename...> typename Event, typename Ret, typename ArgsTuple> struct make_event;
...
template <typename Ret, typename ArgsTuple> struct make_event_handler_fn;

template <typename Ret, typename... Args> struct make_event_handler_fn<Ret, std::tuple<Args...>> {
    using Type = Result<Ret>(const Args &...);
};

template <typename F> struct convert_event_fn {
    using type = F();
};

template <typename Ret, typename... Args> struct convert_event_fn<Ret(Args...)> {
    using type = Ret(Args...);
};

template <typename Listener, typename ArgsTuple> auto invoke_listener(Listener &&listener, ArgsTuple &&args)
{
    return std::apply(listener, std::forward<ArgsTuple>(args));
}

// Specialization for zero arguments
template <typename Listener> auto invoke_listener(Listener &&listener, std::tuple<> &&)
{
    return listener();
}
...
void listen(CallbackFn &&function)
{
    ...
    // CallbackFn is the signature of the callback function. It can be a lambda, a function pointer, or a function
    // object. Its arguments should be the same as EventFn, and the return type is Result<T> where T is the return type
    // of the event.
...
$on_mod(Loaded)
{
    ...
    listen<Type::IsGlobedPlayer, bool(PlayerObject *)>(
        [](PlayerObject *node) { return Ok(typeinfo_cast<globed::VisualPlayer *>(node)); });

    listen<Type::ExplodeRandomPlayer>(
        []() -> Result<void> { return Err("Not implemented; obsolete since globed v2"); });
    ...
    listen<Type::PlayersOnline, size_t>([]() -> Result<size_t> { return Ok(0); });
    ...
    listen<Type::PlayerObjectsForId, std::pair<PlayerObject *, PlayerObject *>(int)>(
        [](int id) -> Result<std::pair<PlayerObject *, PlayerObject *>> {
            auto gjbgl = globed::GlobedGJBGL::get();
            ...
            auto &pl = it->second;
            return Ok(std::make_pair(pl->player1(), pl->player2()));
        });

    // Networking

    listen<Type::IsConnected, bool>([] { return Ok(globed::NetworkManagerImpl::get().isConnected()); });

    listen<Type::ServerTps, uint32_t>([] { return Ok(globed::NetworkManagerImpl::get().getGameTickrate()); });
    ...
    listen<Type::IsStandalone, bool>([] { return Err("Not implemented; obsolete since globed v2"); });
    ...
    listen<Type::IsMod, bool>([] { return Ok(globed::NetworkManagerImpl::get().isModerator()); });

    listen<Type::IsAuthMod, bool>([] { return Ok(globed::NetworkManagerImpl::get().isAuthorizedModerator()); });
    ...
    listen<Type::CbPlayerJoin, void(PlayerJoinFn)>(
        [](PlayerJoinFn fn) -> Result<void> { return Err("Not implemented; obsolete since globed v2"); });

    listen<Type::CbPlayerLeave, void(PlayerLeaveFn)>(
        [](PlayerLeaveFn fn) -> Result<void> { return Err("Not implemented; obsolete since globed v2"); });
    ...
    listen<Type::ActiveSlotContainer, matjson::Value>([] { return Err("Not implemented; obsolete since globed v2"); });

    listen<Type::ReloadSaveContainer, void(matjson::Value)>(
        [](const matjson::Value &container) { return Err("Not implemented; obsolete since globed v2"); });

    listen<Type::LaunchFlag, bool(std::string_view)>(
        [](std::string_view flag) { return Err("Not implemented; obsolete since globed v2"); });
}
