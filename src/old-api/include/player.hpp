...
// Returns whether the given `PlayerObject` belongs to an online player.
// Is significantly faster than `isGlobedPlayer`, but may report false positives or false negatives, as it relies on the
// tag value on the player object. Only use this function if `isGlobedPlayer` is an actual performance concern.
...
{
    return _internal::request<std::pair<PlayerObject *, PlayerObject *>, int>(_internal::Type::PlayerObjectsForId,
                                                                              accountId);
}
} // namespace globed::player
