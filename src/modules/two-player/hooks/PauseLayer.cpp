...
struct GLOBED_MODIFY_ATTR TPHookedPauseLayer : geode::Modify<TPHookedPauseLayer, PauseLayer> {
    static void onModify(auto &self)
    {
        ...
        GLOBED_CLAIM_HOOKS(TwoPlayerModule::get(), self, "PauseLayer::onResume", );
    }

    $override void onResume(CCObject *o)
    {
        ...
        if (!mod.isLinked()) {
            globed::confirmPopup("Globed Error",
                                 "To play in a <cy>2-player mode room</c>, you must <cg>Link</c> to another player. "
                                 "Open the user list menu?",
                                 "Dismiss", "Open", [](auto) { UserListPopup::create()->show(); });
