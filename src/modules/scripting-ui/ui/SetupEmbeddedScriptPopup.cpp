...
bool SetupEmbeddedScriptPopup::setup(TextGameObject *obj)
{
    ...
    auto filenameInput =
        Build(TextInput::create(200.f, "Script name")).pos(this->fromTop(42.f)).parent(m_mainLayer).collect();
    ...
    // editor
    m_editor = Build(CodeEditor::create({400.f, 180.f})).pos(this->fromCenter(0.f, -10.f)).parent(m_mainLayer);
    ...
    Build<CCLabelBMFont>::create("Main", "bigFont.fnt").scale(0.45f).pos(60.f, 24.f).parent(m_mainLayer);
    ...
    Build<CCSprite>::createSpriteName("GJ_myServerBtn_001.png")
        ...
        .intoMenuItem([this] { this->onUpload(); })
        .pos(this->fromBottomRight(24.f, 26.f))
        .scaleMult(1.15f)
        .parent(m_buttonMenu);

    // paste btn
    Build(CircleButtonSprite::createWithSprite("clipboard-icon.png"_spr, 1.f, CircleBaseColor::Pink,
                                               CircleBaseSize::Small))
        .with([&](auto spr) { cue::rescaleToMatch(spr, btnSize); })
        .intoMenuItem([this] { this->onPaste(); })
...
void SetupEmbeddedScriptPopup::onUpload()
{
    auto task = geode::utils::file::pick(
        ...
        file::FilePickOptions{.filters = {{.description = "Lua Script Files (*.lua)", .files = {"*.lua"}}}});
...
void SetupEmbeddedScriptPopup::onClose(CCObject *obj)
{
    if (m_madeChanges && !m_doSave && !m_confirmedExit) {
        globed::confirmPopup("Note", "Are you sure you want to <cr>discard</c> all the changes you've made?", "Cancel",
                             "Discard", [this](auto) {
