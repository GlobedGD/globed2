...
static bool isEmbeddedScript(const gd::string& str) {
    ...
    for (size_t i = 0; i < EMBEDDED_SCRIPT_MAGIC.size(); i++) {
        if (EMBEDDED_SCRIPT_MAGIC[i] != str[i]) return false;
    }
...
Result<std::vector<uint8_t>> EmbeddedScript::encode(bool includePrefix, std::string_view prefixStr) const {
    ...
    MAP_UNWRAP(qn::compressZstd(
        rawData.data(),
        rawData.size(),
        const_cast<uint8_t*>(compressDest.data()),
        bound,
        8
    ));
...
$on_mod(Loaded) {
    ...
    auto test = "VEVTVF9TQ1JJUFQAxBl7-lIAAAAotS_9IFItAgB0AwAJAGhlbGxvLmx1YUEAAABmdW5jdGlvbiBfd29ybGQoKQoJcHJpbnQoIkgsICEiKQplbmQKCgAEAE7d0XkYsFC2Ew8=";
