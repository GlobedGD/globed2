...
struct GLOBED_MODIFY_ATTR UIHookedPauseLayer : Modify<UIHookedPauseLayer, PauseLayer> {
    static void onModify(auto &self)
    {
        ...
        GLOBED_CLAIM_HOOKS(UIModule::get(), self, "PauseLayer::customSetup", "PauseLayer::onQuit", "PauseLayer::onEdit",
                           "PauseLayer::goEdit", "PauseLayer::onResume", "PauseLayer::onNormalMode",
                           "PauseLayer::onPracticeMode", "PauseLayer::onRestart", "PauseLayer::onRestartFull", );
    }
    ...
    $override void customSetup()
    {
        ...
        auto menu =
            Build<CancellableMenu>::create()
                ...
                .layout(
                    ColumnLayout::create()->setAutoScale(false)->setAxisAlignment(AxisAlignment::Start)->setGap(0.f))
                .collect();
        fields.m_rightMenu = menu;
        menu->setTouchCallback([this](bool within) { return this->maybeDismissEmotePopup(); });

        Build<CCSprite>::create("icon-players.png"_spr)
            .scale(0.9f)
            .intoMenuItem(+[] { UserListPopup::create()->show(); })
            .scaleMult(1.2f)
            .id("btn-open-playerlist"_spr)
            .parent(menu);

        if (globed::setting<bool>("core.player.quick-chat-enabled")) {
            auto spr = Build<CCSprite>::create("icon-emotes.png"_spr).scale(0.9f).collect();

            auto btn = HoldableButton::create(
                spr, [](auto) { EmoteListPopup::create()->show(); }, [this](auto) { this->showQuickEmotePopup(); });
    ...
    void showQuickEmotePopup()
    {
        ...
        grid->setLayout(RowLayout::create()->setGap(5.f)->setGrowCrossAxis(true)->setAutoScale(false));
        grid->setZOrder(99);
        ...
        for (size_t i = 0; i < 8; i++) {
            ...
            Build(spr)
                ...
                .intoMenuItem([this, i](auto self) {
                    ...
                    if (emoteId == 0)
                        return; // no emote assigned

                    if (GlobedGJBGL::get()->playSelfEmote(emoteId)) {
                        this->onResume(this);
                    } else {
                        // tint the emoji to red and then back, also shake the button
                        auto tint = CCSequence::create(CCTintTo::create(0.1f, 255, 50, 50),
                                                       CCTintTo::create(0.1f, 255, 255, 255), nullptr);

                        auto shake = CCRepeat::create(CCSequence::create(CCRotateBy::create(0.05f, 15.f),
                                                                         CCRotateBy::create(0.05f, -30.f),
                                                                         CCRotateBy::create(0.05f, 15.f), nullptr),
                                                      2);
        ...
        grid->setScale(0.01f);
        grid->runAction(CCEaseBackOut::create(CCScaleTo::create(0.2f, 1.f)));
    ...
    $override void onQuit(CCObject *sender)
    {
        if (this->hasPopup())
            return;
...
#define REPLACE(method)                                                                                                \
    ...
    $override REPLACE(onEdit);

    $override void onRestart(CCObject *s)
    {
        if (this->hasPopup())
            return;
    ...
    $override void onRestartFull(CCObject *s)
    {
        if (this->hasPopup())
            return;
