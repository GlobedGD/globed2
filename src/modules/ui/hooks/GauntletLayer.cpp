...
struct GLOBED_MODIFY_ATTR HookedGauntletLayer : geode::Modify<HookedGauntletLayer, GauntletLayer> {
    ...
    {
        GLOBED_CLAIM_HOOKS(UIModule::get(), self, "GauntletLayer::init", "GauntletLayer::loadLevelsFinished", );
    }

    $override bool init(GauntletType type)
    {
    ...
    void buildUI()
    {
        ...
        for (size_t i = 0; i < levelButtons.size(); i++) {
            ...
            Build<CCSprite>::create("icon-person.png"_spr).id("level-playercount-icon"_spr).parent(wrapper);

            Build<CCLabelBMFont>::create("", "goldFont.fnt").id("level-playercount-label"_spr).parent(wrapper);
