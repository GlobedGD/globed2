...
void ComplexVisualPlayer::updateData(
    ...
    if (badgeIcon) badgeIcon->setPosition(data.position + ccp(playerName->getScaledContentSize().width / 2.f + 11.5f, 25.f));
	if (statusIcons) {
...
void ComplexVisualPlayer::setForciblyHidden(bool state) {
	if (state) {
		if (playerIcon->m_regularTrail)
			playerIcon->m_regularTrail->setVisible(false);
		if (playerIcon->m_waveTrail)
			playerIcon->m_waveTrail->setVisible(false);
		if (playerIcon->m_ghostTrail)
			playerIcon->m_ghostTrail->setVisible(false);
	}
...
void ComplexVisualPlayer::updateOpacity() {
	...
	if (settings.players.hideNearby) {
		...
		// badgeIcon->setOpacity(playerName->getOpacity()); // FIXME:
	}
}
...
void ComplexVisualPlayer::callToggleWith(PlayerIconType type, bool arg1, bool arg2) {
	...
	case PlayerIconType::Ship:
		playerIcon->toggleFlyMode(arg1, arg2);
		break;
	case PlayerIconType::Ball:
		playerIcon->toggleRollMode(arg1, arg2);
		break;
	case PlayerIconType::Ufo:
		playerIcon->toggleBirdMode(arg1, arg2);
		break;
	case PlayerIconType::Wave:
		playerIcon->toggleDartMode(arg1, arg2);
		break;
	case PlayerIconType::Robot:
		playerIcon->toggleRobotMode(arg1, arg2);
		break;
	case PlayerIconType::Spider:
		playerIcon->toggleSpiderMode(arg1, arg2);
		break;
	case PlayerIconType::Swing:
		playerIcon->toggleSwingMode(arg1, arg2);
		break;
...
void ComplexVisualPlayer::callUpdateWith(PlayerIconType type, int icon) {
	...
	case PlayerIconType::Cube:
		playerIcon->updatePlayerFrame(icon);
		break;
	case PlayerIconType::Ship:
		playerIcon->updatePlayerShipFrame(icon);
		break;
	case PlayerIconType::Ball:
		playerIcon->updatePlayerRollFrame(icon);
		break;
	case PlayerIconType::Ufo:
		playerIcon->updatePlayerBirdFrame(icon);
		break;
	case PlayerIconType::Wave:
		playerIcon->updatePlayerDartFrame(icon);
		break;
	case PlayerIconType::Robot:
		playerIcon->updatePlayerRobotFrame(icon);
		break;
	case PlayerIconType::Spider:
		playerIcon->updatePlayerSpiderFrame(icon);
		break;
	case PlayerIconType::Swing:
		playerIcon->updatePlayerSwingFrame(icon);
		break;
	case PlayerIconType::Jetpack:
		playerIcon->updatePlayerJetpackFrame(icon);
		break;
...
bool ComplexVisualPlayer::isPlayerNearby(const GameCameraState& camState) {
	...
	bool inCameraCoverage = (playerPosition.x >= cameraLeft &&
