...
static bool shouldShowNotification()
{
    ...
    if (typeinfo_cast<CCTransitionScene *>(scene) ||
        (scene->getChildByType<PlayLayer>(0) && !scene->getChildByType<PauseLayer>(0))) {
        return false;
    }
...
void NotificationPanel::slideInNotification(CCNode *node)
{
    ...
    if (this->getChildrenCount() >= 3) {
        ...
        notif->runAction(
            CCSequence::create(CCEaseSineIn::create(CCMoveBy::create(0.6f, {notif->getScaledContentSize().width, 0.f})),
    ...
    for (auto *child : CCArrayExt<CCNode *>(this->getChildren())) {
        ...
        child->runAction(CCEaseSineInOut::create(CCMoveTo::create(0.6f, dest)));
    }
