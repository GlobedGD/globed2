...
static ccColor4B getDefaultColor(size_t idx)
{
    static constexpr ccColor4B colors[] = {
        colorFromHex("#ffffff"), colorFromHex("#fa3939"), colorFromHex("#00A2FF"), colorFromHex("#00FF00"),
        colorFromHex("#FFD300"), colorFromHex("#FF7F00"), colorFromHex("#8F00FF"), colorFromHex("#FF1493"),
        colorFromHex("#00FFFF"), colorFromHex("#228B22"), colorFromHex("#FF4500"), colorFromHex("#FF69B4"),
        colorFromHex("#9932CC"), colorFromHex("#00CED1"), colorFromHex("#B22222"), colorFromHex("#2E8B57"),
        colorFromHex("#F08080"), colorFromHex("#20B2AA"), colorFromHex("#4169E1"), colorFromHex("#708090")};
...
private:
    ...
    bool init(const RoomTeam &team, size_t idx, TeamManagementPopup *popup)
    {
        ...
        auto colorBtn = Build(ColorChannelSprite::create())
                            ...
                            .store(m_colorSprite)
                            .intoMenuItem([this] { this->openEditColor(); })
                            .scaleMult(1.1f)
        ...
        Build<CCLabelBMFont>::create(text.c_str(), "bigFont.fnt").scale(0.6f).parent(leftContainer);
        ...
        if (editable) {
            Build<CCSprite>::createSpriteName("GJ_deleteSongBtn_001.png")
                .with([&](auto spr) { cue::rescaleToMatch(spr, CELL_HEIGHT * 0.8f); })
                .intoMenuItem([this] { m_popup->deleteTeam(m_idx); })
                .zOrder(99)
...
bool TeamManagementPopup::setup(int assigningFor)
{
    ...
    if (m_showPlus) {
        auto toggler = Build(CCMenuItemExt::createTogglerWithStandardSprites(
                                 0.7f, [this](auto toggler) { this->setLockedTeams(!toggler->isOn()); }))
                           .scale(0.9f)
        ...
        Build<CCLabelBMFont>::create("Locked teams", "bigFont.fnt").scale(0.4f).parent(m_bottomContainer);
    }
...
{
    m_loadingCircle = Build(cue::LoadingCircle::create(true)).pos(this->center()).parent(m_mainLayer);
...
void TeamManagementPopup::onTeamCreated(bool success, uint16_t teamCount)
{
    ...
    if (success) {
        ...
        if (teamCount == currentCount + 1) {
            ...
            m_list->addCell(TeamCell::create(RoomTeam{.color = getDefaultColor(teamCount - 1)}, teamCount - 1, this));
...
void TeamManagementPopup::addPlusButton()
{
    ...
    auto plusBtn = Build<CCSprite>::createSpriteName("GJ_plusBtn_001.png")
                       ...
                       .intoMenuItem([this] { this->createTeam(); })
                       .scaleMult(1.1f)
                       .collect();
