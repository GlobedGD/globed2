...
bool SendFeaturePopup::setup(GJGameLevel *level)
{
    ...
    m_menu = Build<CCMenu>::create().pos(this->fromTop(77.f)).parent(m_mainLayer).collect();
    ...
    Build<ButtonSprite>::create("Send", "bigFont.fnt", "GJ_button_01.png", 0.9f)
        .scale(0.75f)
        .intoMenuItem([this] { this->sendLevel(false); })
        .parent(buttonMenu);
    ...
    if (NetworkManagerImpl::get().getUserPermissions().canRateFeatures) {
        Build<ButtonSprite>::create("Queue", "bigFont.fnt", "GJ_button_01.png", 0.9f)
            .scale(0.75f)
            .intoMenuItem([this] { this->sendLevel(true); })
            .parent(buttonMenu);
    }
...
void SendFeaturePopup::sendLevel(bool queue)
{
    ...
    nm.sendSendFeaturedLevel(m_level->m_levelID, m_level->m_levelName, m_level->m_accountID, m_level->m_creatorName,
                             (uint8_t)m_chosenTier, m_noteInput->getString(), queue);
    ...
    Build(globed::createRatingSprite(m_chosenTier))
        ...
        .intoMenuItem([this] {
            ...
            case FeatureTier::Normal:
                m_chosenTier = FeatureTier::Epic;
                break;
            case FeatureTier::Epic:
                m_chosenTier = FeatureTier::Outstanding;
                break;
            case FeatureTier::Outstanding:
            default:
                m_chosenTier = FeatureTier::Normal;
                break;
            }
