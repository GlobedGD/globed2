...
public:
    static PlayerListCell *create(int accountId, int userId, const std::string &username, const cue::Icons &icons,
                                  const std::optional<SpecialUserData> &sud, cocos2d::CCSize cellSize)
    {
...
protected:
    bool customSetup() override
    {
        ...
        Build<CCSprite>::create("icon-invite.png"_spr)
            ...
            .intoMenuItem([this] { NetworkManagerImpl::get().sendInvitePlayer(m_accountId); })
            .zOrder(9)
            .scaleMult(1.1f)
            .parent(m_rightMenu);
...
bool InvitePopup::setup()
{
    ...
    Build<CCSprite>::create("refresh01.png"_spr)
        .with([&](auto spr) { cue::rescaleToMatch(spr, btnSize); })
        .intoMenuItem([this] { this->refresh(); })
        .zOrder(9)
        .scaleMult(1.1f)
        .id("refresh-btn")
...
void InvitePopup::onLoaded(const std::vector<MinimalRoomPlayer> &players)
{
    ...
    for (auto &player : players) {
        m_list->addCell(PlayerCell::create(player.accountData.accountId, player.accountData.userId,
                                           player.accountData.username, player.toIcons(), std::nullopt, CELL_SIZE));
    }
