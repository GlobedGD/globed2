...
public:
    static PlayerListCell *create(int accountId, int userId, const std::string &username, const cue::Icons &icons,
                                  const std::optional<SpecialUserData> &sud, SessionId sessionId)
    {
    ...
    void recreateButtons()
    {
        ...
        if (m_sessionId.asU64() != 0) {
            ...
            m_joinBtn = Build<CCSprite>::createSpriteName("GJ_playBtn2_001.png")
                            ...
                            .intoMenuItem([this] { globed::warpToSession(m_sessionId, false, true); })
                            .zOrder(10)
                            .scaleMult(1.1f)
                            .parent(m_rightMenu);
        }

        // if we are room host or a moderator, add a button that allows us kicking/banning the person or opening mod
        // panel
        if (isInRoom && (isMod || isOwner)) {
            m_modBtn = Build<CCSprite>::createSpriteName("GJ_reportBtn_001.png")
                           .with([&](auto spr) { cue::rescaleToMatch(spr, BTN_SIZE); })
                           .intoMenuItem([this] { this->openUserControls(); })
                           .zOrder(9)
                           .scaleMult(1.1f)
                           .parent(m_rightMenu);
        } else if (isMod) {
            m_modBtn = Build<CCSprite>::createSpriteName("GJ_reportBtn_001.png")
                           .with([&](auto spr) { cue::rescaleToMatch(spr, BTN_SIZE); })
                           .intoMenuItem([this] { this->openModPanel(); })
                           .zOrder(9)
                           .scaleMult(1.1f)
                           .parent(m_rightMenu);
        }
...
bool GlobedMenuLayer::init()
{
    ...
    Build<CCMenu>::create()
        ...
        .child(Build<CCLabelBMFont>::create("Server Name", "goldFont.fnt").store(m_serverNameLabel))
        .child(Build<CCSprite>::create("pencil.png"_spr)
                   .scale(0.7f)
                   .intoMenuItem([this] { ServerListPopup::create(this)->show(); })
    ...
    m_connectButton = Build<ButtonSprite>::create("Connect", "bigFont.fnt", "GJ_button_01.png", 0.7f)
                          ...
                          .intoMenuItem([] {
                              ...
                              if (!argon::signedIn()) {
                                  globed::confirmPopup("Globed Error",
                                                       "You must be logged into a <cg>Geometry Dash account</c> in "
                                                       "order to play online. Want to visit the <cy>account page</c>?",
                                                       "Cancel", "Ok", [](auto) {
    ...
    m_connStateLabel =
        Build<CCLabelBMFont>::create("", "bigFont.fnt").scale(0.6f).id("conn-state-label").parent(m_connStateContainer);

    // cancel connection button
    m_cancelConnButton = Build<CCSprite>::create("exit01.png"_spr)
                             .with([&](auto spr) { cue::rescaleToMatch(spr, 27.5f); })
                             .intoMenuItem([] { NetworkManagerImpl::get().disconnectCentral(); })
                             .id("cancel-conn-btn")
    ...
    Build<CCSprite>::create("settings01.png"_spr)
        ...
        .intoMenuItem([this] { this->onSettings(); })
        .scaleMult(1.1f)
        .parent(buttonMenu);
    ...
    m_roomNameButton = Build<Label>::create("", "goldFont.fnt")
                           ...
                           .intoMenuItem([this] { this->copyRoomIdToClipboard(); })
                           .scaleMult(1.1f)
                           .pos(PLAYER_LIST_MENU_SIZE.width / 2.f, PLAYER_LIST_MENU_SIZE.height - 16.f)
                           .id("room-name-btn")
                           .parent(m_playerListMenu);

    m_playerList = Build<cue::ListNode>::create(PLAYER_LIST_SIZE, ccColor4B{0x33, 0x44, 0x99, 255},
    ...
    m_playerList->setCellColors(ccColor4B{0x28, 0x35, 0x77, 255}, ccColor4B{0x33, 0x44, 0x99, 255});
    ...
    auto colLayout = ColumnLayout::create()->setAutoScale(false)->setGap(3.f)->setAxisReverse(true)->setAxisAlignment(
        AxisAlignment::End);
    ...
    auto farLayout =
        ColumnLayout::create()->setAutoScale(false)->setAxisReverse(true)->setAxisAlignment(AxisAlignment::Start);
...
void GlobedMenuLayer::initNewRoom(uint32_t id, const std::string &name, const std::vector<RoomPlayer> &players,
                                  size_t playerCount, const RoomSettings &settings)
{
    m_roomId = id;
...
void GlobedMenuLayer::updateRoom(uint32_t id, const std::string &name, const std::vector<RoomPlayer> &players,
                                 size_t playerCount, const RoomSettings &settings)
{
...
void GlobedMenuLayer::updatePlayerList(const std::vector<RoomPlayer> &players)
{
    ...
    for (auto &player : sortedPlayers) {
        m_playerList->addCell(PlayerCell::create(player.accountData.accountId, player.accountData.userId,
                                                 ...
                                                 player.specialUserData, player.session));
    }
...
void GlobedMenuLayer::addPinnedLevelCell()
{
    ...
    if (!cell) {
        ...
        cell->setUpdateCallback([this] { m_playerList->updateLayout(); });
...
bool GlobedMenuLayer::trySoftRefresh(const std::vector<RoomPlayer> &players)
{
    ...
    for (auto &pl : players) {
        if (pl.accountData.accountId == selfId)
            continue;
...
void GlobedMenuLayer::initRoomButtons()
{
    ...
    if (m_roomId == 0) {
        ...
        Build(ButtonSprite::create("Join Room", "bigFont.fnt", "GJ_button_01.png", BtnScale))
            .intoMenuItem(+[] {
                if (auto p = RoomListingPopup::create())
                    p->show();
            })
            .scaleMult(1.1f)
        ...
        Build(ButtonSprite::create("Create Room", "bigFont.fnt", "GJ_button_01.png", BtnScale))
            .intoMenuItem(+[] { CreateRoomPopup::create()->show(); })
            .scaleMult(1.1f)
            .parent(m_roomButtonsMenu);
    } else {
        Build(ButtonSprite::create("Leave Room", "bigFont.fnt", "GJ_button_01.png", BtnScale))
            .intoMenuItem(+[] { NetworkManagerImpl::get().sendLeaveRoom(); })
            .scaleMult(1.1f)
            .parent(m_roomButtonsMenu);
    }
...
void GlobedMenuLayer::initSideButtons()
{
    ...
    auto makeButton = [](CCSprite *sprite, CCNode *parent, int zOrder, const char *id,
                         auto cb) -> CCMenuItemSpriteExtra * {
        if (!sprite) {
        ...
        return Build(sprite)
            ...
            .intoMenuItem([cb = std::move(cb)](auto) { cb(); })
            .zOrder(zOrder)
            .scaleMult(1.1f)
            .id(id)
            .parent(parent)
    ...
    makeButton(CCSprite::create("exit02.png"_spr), m_leftSideMenu, LeftBtn::Disconnect, "btn-disconnect", [] {
        globed::confirmPopup("Note", "Are you sure you want to <cr>disconnect</c> from the server?", "Cancel", "Yes",
                             [](auto) { NetworkManagerImpl::get().disconnectCentral(); });
    });

    // region switching button
    makeButton(CCSprite::create("server02.png"_spr), m_leftSideMenu, LeftBtn::RegionSwitch, "btn-region-select", [] {
    ...
    if (rm.getSettings().teams) {
        makeButton(CCSprite::create("teams02.png"_spr), m_leftSideMenu, LeftBtn::Teams, "btn-manage-teams",
                   [] { TeamManagementPopup::create(0)->show(); });
    }

    if (rm.isOwner()) {
        auto btn = makeButton(CCSprite::create("settings01.png"_spr), m_leftSideMenu, LeftBtn::Settings, "btn-settings",
                              [] { RoomSettingsPopup::create()->show(); });
    }

    // close room button
    if (!rm.isInGlobal() && (rm.isOwner() || NetworkManagerImpl::get().isAuthorizedModerator())) {
        makeButton(CCSprite::create("exit01.png"_spr), m_leftSideMenu, LeftBtn::CloseRoom, "btn-close-room", [] {
            globed::confirmPopup("Close Room",
                                 "Are you sure you want to <cr>close</c> the room? All players will be <cy>kicked</c> "
                                 "from the room and it will be <cy>deleted</c>.",
                                 "Cancel", "Ok", [](auto) {
    ...
    makeButton(CCSprite::create("privacySettings.png"_spr), m_rightSideMenu, RightBtn::PrivacySettings,
               "btn-privacy-settings", [] { UserSettingsPopup::create()->show(); });
    ...
    if (NetworkManagerImpl::get().isModerator()) {
        ...
        if (badge) {
            ...
            makeButton(wrapper, m_rightSideMenu, RightBtn::AdminPanel, "btn-managemod-panel",
                       [] { globed::openModPanel(); });
    ...
    if (!rm.isInGlobal() && (rm.isOwner() || !rm.getSettings().privateInvites)) {
        makeButton(CCSprite::create("invite.png"_spr), m_rightSideMenu, RightBtn::Invite, "btn-invite",
                   [] { InvitePopup::create()->show(); });
    }

    // filter button
    m_searchBtn =
        makeButton(CCSprite::create("search01.png"_spr), m_rightSideMenu, RightBtn::Search, "btn-search", [this] {
    ...
    // clear filter button
    m_clearSearchBtn = makeButton(CCSprite::create("search02.png"_spr), m_rightSideMenu, RightBtn::ClearSearch,
                                  "btn-clear-search", [this] { this->reloadWithFilter(""); });
    m_clearSearchBtn->setVisible(false);

    // refresh button
    makeButton(CCSprite::create("refresh01.png"_spr), m_rightSideMenu, RightBtn::Refresh, "btn-refresh", [this] {
        m_hardRefresh = true;
...
void GlobedMenuLayer::initFarSideButtons()
{
    ...
    Build<CCSprite>::create("levels01.png"_spr)
        .with([&](auto btn) { cue::rescaleToMatch(btn, FAR_BTN_SIZE); })
        .intoMenuItem([] { LevelListLayer::create()->switchTo(); })
        .scaleMult(1.1f)
    ...
    if (flevel) {
        ...
        if (isNew) {
            Build<CCSprite>::createSpriteName("newMusicIcon_001.png")
                ...
                .parent(fbutton)
                .with([&](auto spr) {
                    spr->runAction(CCRepeatForever::create(
                        CCSequence::create(CCEaseSineInOut::create(CCScaleTo::create(0.75f, 1.2f)),
                                           CCEaseSineInOut::create(CCScaleTo::create(0.75f, 1.0f)), nullptr)));
                });
        }
    }
...
std::vector<Ref<CCMenuItemSpriteExtra>> GlobedMenuLayer::createCommonButtons()
{
    ...
    out.push_back(Build<CCSprite>::create("support01.png"_spr)
                      .with([&](auto btn) { cue::rescaleToMatch(btn, FAR_BTN_SIZE); })
                      .intoMenuItem([] { CreditsPopup::create()->show(); })
                      .scaleMult(1.1f)
    ...
    out.push_back(Build<CCSprite>::create("support02.png"_spr)
                      .with([&](auto btn) { cue::rescaleToMatch(btn, FAR_BTN_SIZE); })
                      .intoMenuItem([] { SupportPopup::create()->show(); })
                      .scaleMult(1.1f)
    ...
    out.push_back(Build<CCSprite>::create("discord01.png"_spr)
                      .with([&](auto btn) { cue::rescaleToMatch(btn, FAR_BTN_SIZE); })
                      .intoMenuItem([] {
                          globed::confirmPopup("Open Discord",
                                               "Join our <cp>Discord</c> server?\n\n<cr>Important: By joining the "
                                               "Discord server, you agree to being at least 13 years of age.</c>",
                                               "No", "Yes", [](auto) {
