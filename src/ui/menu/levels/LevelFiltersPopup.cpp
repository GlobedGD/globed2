...
bool LevelFiltersPopup::setup(LevelListLayer *layer)
{
    ...
    auto *togglesMenu = Build<CCMenu>::create()
                            .layout(RowLayout::create()->setAutoScale(false)->setGrowCrossAxis(true)->setAxisAlignment(
                                AxisAlignment::Start))
                            .parent(m_mainLayer)
    ...
    auto addToggle = [&](const char *label, bool initial, auto callback) {
        ...
        Build<CCMenu>::create()
            ...
            .layout(RowLayout::create()->setAutoScale(false)->setGap(7.f)->setAxisReverse(true)->setAxisAlignment(
                AxisAlignment::Start))
            .parent(togglesMenu)
            .child(Build<CCLabelBMFont>::create(label, "bigFont.fnt").scale(0.3f).collect())
            .intoNewChild(CCMenuItemExt::createTogglerWithStandardSprites(
                0.65f, [callback = std::move(callback)](auto btn) { callback(!btn->isToggled()); }))
            .store(toggler)
    ...
    addToggle("Original", m_filters.original, [this](bool state) { m_filters.original = state; });

    addToggle("Coins", m_filters.coins && *m_filters.coins,
              [this](bool state) { m_filters.coins = state ? std::optional{true} : std::nullopt; });

    addToggle("2-Player", m_filters.twoPlayer, [this](bool state) { m_filters.twoPlayer = state; });
    ...
    addToggle("No Star", m_filters.rateTier && m_filters.rateTier->contains(Unrated),
              [modifyRateTier](bool state) { modifyRateTier(Unrated, state); });

    addToggle("Featured", m_filters.rateTier && m_filters.rateTier->contains(Feature),
              [modifyRateTier](bool state) { modifyRateTier(Feature, state); });

    addToggle("Epic", m_filters.rateTier && m_filters.rateTier->contains(Epic),
              [modifyRateTier](bool state) { modifyRateTier(Epic, state); });

    addToggle("Legendary", m_filters.rateTier && m_filters.rateTier->contains(Legendary),
              [modifyRateTier](bool state) { modifyRateTier(Legendary, state); });

    addToggle("Mythic", m_filters.rateTier && m_filters.rateTier->contains(Mythic),
              [modifyRateTier](bool state) { modifyRateTier(Mythic, state); });
    ...
    Build<CCSprite>::createSpriteName("GJ_timeIcon_001.png").parent(lengthMenu);

    for (auto [name, len] : std::initializer_list<std::tuple<const char *, int>>{
             {"Tiny", 0}, {"Short", 1}, {"Medium", 2}, {"Long", 3}, {"XL", 4}, {"Plat.", 5}}) {
