...
static void onDescriptionClicked(std::string_view key)
{
    if (key == "core.user.hide-in-levels") {
        ...
        globed::alertFormat("Hide in Menus",
                            "When enabled, other players won't be able to see you in menus and level lists.");
    } else if (key == "core.user.hide-roles") {
        globed::alertFormat("Hide Roles", "When enabled, other players won't be able to see your special roles (e.g. "
                                          "Moderator) in levels and menus.");
    } else {
...
{
    auto toggler =
        CCMenuItemExt::createToggler(CCSprite::create(onSprite), CCSprite::create(offSprite), [this, key](auto btn) {
            if (key == "core.user.hide-in-levels" && !globed::swapFlag("core.flags.seen-hide-in-levels-note")) {
                globed::alertFormat("Warning",
    ...
    Build<CCMenu>::create()
        ...
        .child(toggler)
        .parent(m_menu)
        .intoNewChild(Build<CCSprite>::createSpriteName("GJ_infoIcon_001.png")
                          .scale(0.5f)
                          .intoMenuItem([key] { onDescriptionClicked(key); })
