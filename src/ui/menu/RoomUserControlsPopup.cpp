...
void RoomUserControlsPopup::remakeButtons()
{
    ...
    Build<CCSprite>::create("button-admin-ban.png"_spr)
        ...
        .intoMenuItem([this] {
            globed::confirmPopup("Ban user",
                                 fmt::format("Are you sure you want to <cr>ban</c> <cy>{}</c> from your room? They "
                                             "will be unable to join until you create a new room.",
                                             m_username),
                                 "Cancel", "Ban", [this](auto) {
                                     this->onClose(nullptr);

                                     NetworkManagerImpl::get().sendRoomOwnerAction(RoomOwnerActionType::BAN_USER,
                                                                                   m_accountId);
                                 });
        })
        .scaleMult(1.1f)
    ...
    Build<CCSprite>::create("button-admin-kick.png"_spr)
        ...
        .intoMenuItem([this] {
            globed::confirmPopup(
                "Kick user",
                fmt::format(
                    "Are you sure you want to <cr>kick</c> <cy>{}</c> from your room? They will be able to join again.",
                    m_username),
                "Cancel", "Kick", [this](auto) {
                    this->onClose(nullptr);
    ...
    if (rm.getSettings().teams) {
        Build(EditorButtonSprite::createWithSprite("icon-person.png"_spr, 1.f, EditorBaseColor::Cyan))
            .with([&](auto spr) { cue::rescaleToMatch(spr, btnSize); })
            .intoMenuItem([this] { TeamManagementPopup::create(m_accountId)->show(); })
            .scaleMult(1.1f)
            .parent(m_menu);
    }
    ...
    if (NetworkManagerImpl::get().isAuthorizedModerator()) {
        Build<CCSprite>::createSpriteName("GJ_reportBtn_001.png")
            .with([&](auto spr) { cue::rescaleToMatch(spr, btnSize); })
            .intoMenuItem([this] { ModUserPopup::create(m_accountId)->show(); })
            .scaleMult(1.1f)
            .parent(m_menu);
    }
