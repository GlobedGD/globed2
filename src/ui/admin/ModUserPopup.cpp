...
void ModUserPopup::initUi()
{
    ...
    Build<CCLabelBMFont>::create(m_score->m_userName.c_str(), "bigFont.fnt")
        ...
        .intoMenuItem([this] { globed::openUserProfile(m_score->m_accountID, m_score->m_userID, m_score->m_userName); })
        .scaleMult(1.15f)
        .parent(m_nameLayout)
        .collect();
    ...
    m_rootMenu = Build<CCMenu>::create()
                     ...
                     .parent(m_rootLayout)
                     .layout(RowLayout::create()->setGap(5.f)->setAutoScale(false)->setGrowCrossAxis(true));
    ...
    Build<CCSprite>::create(m_data->whitelisted ? "button-admin-unwhitelist.png"_spr : "button-admin-whitelist.png"_spr)
        ...
        .intoMenuItem([this](CCMenuItemSpriteExtra *btn) {
            globed::confirmPopup("Confirm",
                                 m_data->whitelisted ? "Are you sure you want to remove this person from the whitelist?"
                                                     : "Are you sure you want to whitelist this person?",
                                 "Cancel", "Yes", [this, btn](auto) {
                                     bool newv = !m_data->whitelisted;
                                     NetworkManagerImpl::get().sendAdminSetWhitelisted(m_data->accountId, newv);
                                     m_data->whitelisted = newv;

                                     btn->setSprite(Build<CCSprite>::create(newv ? "button-admin-unwhitelist.png"_spr
                                                                                 : "button-admin-whitelist.png"_spr)
                                                        .scale(btnScale));
                                 });
        })
    ...
    Build<CCSprite>::create("button-admin-kick.png"_spr)
        .scale(btnScale)
        .intoMenuItem([this] {
            globed::confirmPopup("Confirm", "Are you sure you want to <cr>kick</c> this person from the server?",
...
void ModUserPopup::recreateRoleButton()
{
    ...
    m_roleModifyButton = Build(badgeIcon)
                             .intoMenuItem([this] {
                                 if (!NetworkManagerImpl::get().getUserPermissions().canEditRoles)
                                     return;

                                 auto popup = ModRoleModifyPopup::create(m_data->accountId, m_data->roles);
                                 popup->setCallback([this] { this->fullRefresh(); });
                                 popup->show();
                             })
                             .parent(m_nameLayout);
...
void ModUserPopup::createMuteAndBanButtons()
{
    ...
    m_banButton = Build<CCSprite>::create(m_data->activeBan ? "button-admin-unban.png"_spr : "button-admin-ban.png"_spr)
                      .scale(btnScale)
                      .intoMenuItem([this] { this->showPunishmentPopup(UserPunishmentType::Ban); })
                      .zOrder(btnorder::Ban)
                      .parent(m_rootMenu);
    ...
    // Mute button
    m_muteButton =
        Build<CCSprite>::create(m_data->activeMute ? "button-admin-unmute.png"_spr : "button-admin-mute.png"_spr)
            .with([&](CCSprite *btn) { cue::rescaleToMatch(btn, expSize); })
            .intoMenuItem([this] { this->showPunishmentPopup(UserPunishmentType::Mute); })
            .zOrder(btnorder::Mute)
            .parent(m_rootMenu);

    // Room ban button
    m_roomBanButton = Build<CCSprite>::create(m_data->activeRoomBan ? "button-admin-room-unban.png"_spr
                                                                    : "button-admin-room-ban.png"_spr)
                          .with([&](CCSprite *btn) { cue::rescaleToMatch(btn, expSize); })
                          .intoMenuItem([this] { this->showPunishmentPopup(UserPunishmentType::RoomBan); })
...
{
    auto &active = type == UserPunishmentType::Ban    ? m_data->activeBan
...
void ModUserPopup::startLoadingProfile(const std::string &query, bool isId)
{
    ...
    m_loadCircle = Build<cue::LoadingCircle>::create().pos(this->center()).parent(m_mainLayer);
...
{
    NetworkManagerImpl::get().sendAdminUpdateUser(m_data->accountId, m_score->m_userName, m_score->m_playerCube,
                                                  m_score->m_color1, m_score->m_color2,
