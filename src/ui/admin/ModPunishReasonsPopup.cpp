...
private:
    ...
    bool init(const std::string &reason)
    {
        ...
        m_label =
            Build<Label>::create("", "bigFont.fnt").anchorPoint(0.f, 0.5f).pos(5.f, CELL_HEIGHT / 2.f).parent(this);
        ...
        Build<CCSprite>::createSpriteName("btn_chatHistory_001.png")
            ...
            .intoMenuItem([this] { m_popup->invokeCallback(m_reason); })
            .parent(m_rightMenu);
    ...
    {
        // edit button
        auto spr = Build<CCSprite>::create("pencil.png"_spr).collect();

        Build<CircleButtonSprite>::create(spr)
            .with([&](auto *item) { cue::rescaleToMatch(item, BUTTON_SIZE); })
            .intoMenuItem([this](auto) { m_popup->openEditReasonPopup(this->getIndex(), m_reason); })
            .parent(m_rightMenu);

        // delete button
        Build<CCSprite>::createSpriteName("GJ_deleteSongBtn_001.png")
            .with([&](auto spr) { cue::rescaleToMatch(spr, BUTTON_SIZE); })
            .intoMenuItem([this] { m_popup->deleteCustomReason(this->getIndex()); })
            .parent(m_rightMenu);
    }
...
bool ModPunishReasonsPopup::setup(UserPunishmentType type)
{
    ...
    Build<Label>::create("Custom", "bigFont.fnt").scale(0.45f).parent(btnContainer);
...
{
    auto list = Build(cue::ListNode::create(LIST_SIZE)).pos(this->center()).parent(m_mainLayer).collect();
    ...
    if (custom) {
        auto plusBtn = Build<CCSprite>::createSpriteName("GJ_plusBtn_001.png")
                           ...
                           .intoMenuItem([this] { this->createBlankReason(); })
                           .scaleMult(1.1f)
                           .collect();
...
{
    this->setTitle(custom ? "Custom Reasons" : "Common Reasons", "goldFont.fnt", 0.7f, 17.5f);
