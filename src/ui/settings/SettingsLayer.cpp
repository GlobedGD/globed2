...
bool SettingsLayer::init()
{
    ...
    constexpr static auto names = std::array{
        "Globed", "Players", "Level UI", "Voice", "Menus",
    };
    ...
    Build<CircleButtonSprite>::create(CCSprite::create("icon-folder-settings.png"_spr), CircleBaseColor::Pink)
        ...
        .intoMenuItem([this](auto) {
            ...
            popup->setSwitchCallback([this] { this->refreshAll(); });
            popup->show();
        })
        .id("btn-save-slots")
        .parent(rightMenu);
...
void SettingsLayer::addSettings()
{
    ...
    this->addSetting<BoolSettingCell>(
        "core.autoconnect", "Autoconnect",
    ...
    this->addSetting(ButtonSettingCell::create(
        "Discord Linking",
        "Link your <cb>Discord</c> account to receive <cg>roles</c> (for example <ca>Supporter</c>, <cp>Booster</c>, "
        "<cg>Moderator</c>, etc.) and unlock <cy>voice chat</c>.",
        "Link",
        [] {
        ...
        CELL_SIZE));
    this->addSetting(ButtonSettingCell::create(
        "Keybinds", "Configure in-game Globed keybinds.", "Edit", [] { KeybindsPopup::create()->show(); }, CELL_SIZE));
    this->addSetting<BoolSettingCell>(
        "core.editor.enabled", "Show Players in Editor",
        "Hosts local (editor) levels as well, letting you see other players playing the level while you are in the "
        "editor. Note: <cy>this does not let you build levels together!</c>");
    ...
    this->addSetting<BoolSettingCell>("core.ui.disable-notices", "Disable Notices",
                                      "Entirely disables <cy>Notices</c>, which are a way for <cj>Globed staff</c> to "
    ...
    this->addSetting<BoolSettingCell>("core.preload.enabled", "Preload Assets",
                                      "Loads <cy>player assets</c> (icons, death effects) in advance to reduce <cr>lag "
                                      "spikes</c> in levels. This will use extra <cg>memory</c> and will increase "
                                      "<cy>load times</c> unless <cj>Defer Preloading</c> is enabled.");
    this->addSetting<BoolSettingCell>("core.preload.defer", "Defer Preloading",
                                      "Instead of preloading all assets on the loading screen, they are only loaded "
                                      "when joining a level (while connected to server).");

    // Advanced settings
    this->addHeader("core.dev", "Advanced", m_globedTab);
    this->addSetting<BoolSettingCell>(
        "core.ui.allow-custom-servers", "Allow Custom Servers",
    ...
    this->addSetting<BoolSettingCell>(
        "core.dev.cert-verification", "SSL Verification",
        "Enables SSL certificate verification when connecting to servers. Disabling this is <cr>not recommended</c> in "
        "general, unless experiencing connection issues. This will only affect <cy>Argon authentication</c>, "
        "<cy>QUIC/WS</c> connections and <cy>DoH</c>, not TCP or UDP.");

    if (showDebug) {
        this->addSetting<FloatSettingCell>(
            "core.dev.packet-loss-sim", "Packet Loss Simulation",
            "Artificially simulate outbound packet loss for testing purposes. Results depend on the transport: for "
            "<cy>TCP</c> the data is permanently lost, for <cy>UDP</c> the data may be resent if it was a reliable "
            "message, for <cy>QUIC</c> there's no loss as it will be handled by the protocol.");
        this->addSetting<BoolSettingCell>(
            "core.dev.net-stat-dump", "Network Stat Dump",
            "Enables highly detailed network statistics dumping for debugging. This will write very detailed "
            "information about all inbound and outbound packets to a log file, and dump exact packet bytes. F7 can be "
            "pressed while in the Globed menu to show current connection stats. <cy>This will use a lot of "
            "memory</c>.");
        this->addSetting<BoolSettingCell>(
            "core.dev.fake-data", "Use Fake Data",
        ...
        this->addSetting(ButtonSettingCell::create(
            "Clear Auth Tokens", "Deletes all stored authentication tokens for all servers.", "Clear",
            [] { NetworkManagerImpl::get().clearAllUTokens(); }, CELL_SIZE));

        this->addSetting(ButtonSettingCell::create(
            "Clear Argon Token", "Deletes the stored Argon authentication token.", "Clear", [] { argon::clearToken(); },
            CELL_SIZE));
    }
    ...
    this->addSetting<BoolSettingCell>(
        "core.player.quick-chat-enabled", "Emotes",
        "Enables the use of <cy>emotes</c> (quick chat) in levels. They can be accessed via the <cg>pause menu</c> and "
        "then bound to <cr>keybinds</c> (by default numbers 1-4).");
    this->addSetting<BoolSettingCell>(
        "core.player.quick-chat-sfx", "Emote Sound Effects",
        "Plays sound effects when you or other players use certain <cy>emotes</c> in levels.");
    this->addSetting<EmoteVolumeCell>(
        "core.player.quick-chat-sfx-volume", "Emote SFX Volume",
    ...
    this->addSetting<BoolSettingCell>(
        "core.player.status-icons", "Show Status Icons",
    ...
    this->addSetting<BoolSettingCell>("core.level.self-name", "Show Own Name", "Show your own name in levels.");

    // Level UI
    this->addHeader("core.level", "Level UI", m_levelUiTab);
    this->addSetting<BoolSettingCell>(
        "core.level.progress-indicators", "Progress Markers (Classic)",
    ...
    this->addSetting<BoolSettingCell>("core.level.progress-indicators-plat", "Direction Markers (Plat)",
                                      "Show icons near edges of the screen indicating where other players are located "
                                      "(if offscreen) in <cg>platformer levels</c>.");
    ...
    this->addSetting<BoolSettingCell>("core.level.force-progressbar", "Force Progress Bar",
                                      "Forces the <cg>progress bar</c> to be shown in classic levels when connected to "
                                      "Globed, even if the user has it disabled.");
    ...
    this->addSetting<BoolSettingCell>("core.overlay.always-show", "Always Show Overlay",
                                      "Show the <cy>ping overlay</c> even when not connected or in unsupported levels, "
                                      "replacing the ping with a text like <cr>'Not connected'</c> or <cr>'N/A'</c>.");

    // Audio
    this->addHeader("core.audio", "Audio", m_voiceTab);
    this->addSetting<BoolSettingCell>(
        "core.audio.voice-chat-enabled", "Voice Chat",
        "Enable in-game voice chat (default keybind is V). Note: <cy>this is currently only supported on Windows, and "
        "requires you to link your </c><cb>Discord</c> <cy>account</c>.");
    this->addSetting<FloatSettingCell>("core.audio.playback-volume", "Voice Volume",
                                       "Adjust the global voice chat volume.");
    this->addSetting(ButtonSettingCell::create(
        "Audio Device", "Sets the used input device for voice chat.", "Set",
        [] {
            ...
            globed::alert("Error",
                          ...
                          "Microphone support is currently <cr>not available</c> on this platform, and is unlikely to "
                          "be added in the future due to macOS restrictions."
#else
    ...
    this->addSetting<BoolSettingCell>("core.audio.voice-proximity", "Voice Proximity (Plat)",
                                      "Enables proximity voice chat in <cg>platformer</c> levels, meaning players get "
                                      "quieter the further away they are from you.");
    this->addSetting<BoolSettingCell>("core.audio.classic-proximity", "Voice Proximity (Classic)",
                                      "Enables proximity voice chat in <cg>classic</c> levels, meaning players get "
                                      "quieter the further away they are from you.");
    ...
    this->addSetting<BoolSettingCell>(
        "core.audio.overlaying-overlay", "Overlay On Top",
        "Render the voice overlay above other UI elements, allowing it to be used in the <cg>pause menu</c>.");
    this->addSetting<IntSliderSettingCell>(
        "core.audio.buffer-size", "Audio Buffer Size",
    ...
    this->addSetting<BoolSettingCell>("core.ui.compressed-player-count", "Simple Player Count",
                                      "Shows a simplified player count (number + icon rather than 'x players') in "
                                      "<cg>all places</c> instead of just <cy>Tower/Gauntlets</c>.");
