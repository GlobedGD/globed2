...
bool DiscordLinkAttemptPopup::setup(uint64_t userId, const std::string &username, const std::string &avatarUrl)
{
    ...
    auto avatar = Build(LazySprite::create({40.f, 40.f})).parent(card).collect();
    ...
    Build<CCMenu>::create()
        ...
        .parent(m_mainLayer)
        .child(Build<ButtonSprite>::create("Decline", "bigFont.fnt", "GJ_button_06.png", 0.8f)
                   .scale(0.8f)
                   .intoMenuItem([this] { this->confirm(false); })
                   .scaleMult(1.1f))
        .child(Build<ButtonSprite>::create("Confirm", "bigFont.fnt", "GJ_button_01.png", 0.8f)
                   .scale(0.8f)
                   .intoMenuItem([this] { this->confirm(true); })
                   .scaleMult(1.1f))
        .updateLayout();
