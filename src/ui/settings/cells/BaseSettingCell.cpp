...
bool BaseSettingCellBase::init(CStr key, CStr name, CStr desc, cocos2d::CCSize size)
{
    m_key = key;
...
bool BaseSettingCellBase::initNoSetting(CStr name, CStr desc, cocos2d::CCSize size)
{
    ...
    if (!desc.empty()) {
        Build<CCSprite>::createSpriteName("GJ_infoIcon_001.png")
            .scale(0.45f)
            .intoMenuItem([this] { globed::alert(m_name, std::string(m_desc)); })
            .pos(8.f + label->getScaledContentWidth() + 6.f,
                 size.height / 2.f + label->getScaledContentHeight() / 2.f - 4.f)
            .parent(this);
    }
