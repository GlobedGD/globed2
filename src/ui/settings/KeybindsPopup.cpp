...
protected:
    ...
    void init(CStr key, CStr name, CStr desc)
    {
        ...
        Build<CircleButtonSprite>::create(spr)
            .with([&](auto *item) { cue::rescaleToMatch(item, CELL_HEIGHT * 0.7f); })
            .intoMenuItem([this](auto) {
                auto popup = KeybindSetupPopup::create(this->getKey());
                popup->setCallback([this](auto key) { this->setKey(key); });
                popup->show();
            })
            .parent(m_menu);
    ...
    void updateKey(enumKeyCodes key)
    {
        ...
        m_keyWrapper = Build<CCNode>::create().parent(m_menu);
...
bool KeybindsPopup::setup()
{
    ...
    m_list->addCell(
        KeybindCell::create("core.keybinds.voice-chat", "Voice activate",
                            "Captures microphone input and sends it to other users while the button is held"));

    m_list->addCell(
        KeybindCell::create("core.keybinds.deafen", "Deafen", "Temporarily disables ability to hear other players"));
#endif

    m_list->addCell(KeybindCell::create("core.keybinds.hide-players", "Hide players",
                                        "Hide / unhide all players while in a level"));

#define EMOTE_CELL(n, human)                                                                                           \
    m_list->addCell(KeybindCell::create("core.keybinds.emote-" n, "Emote " human,                                      \
