...
// Can be constructed from a `std::string` or a `const char*`. It is not safe to use `string_view.data()` to construct
// this. Can never hold a null pointer, converts it into an empty string pointer.
class CStr {
public:
    inline CStr() : m_ptr("") {}
