...
enum class ListenerResult {
    ...
    Continue,
    /// Stop executing the listener chain, if a listener returns this, it will be the last listener to be called with
    /// this message.
    Stop,
};

template <typename T> using ListenerFn = std23::move_only_function<ListenerResult(const T &)>;
...
template <typename T> class MessageListenerImpl : public MessageListenerImplBase {
public:
    ...
    /// Set whether the listener is thread-safe. If `false` (the default), the listener will always be called on the
    /// main thread.
...
template <typename T> class MessageListener {
public:
    MessageListener(const MessageListener &) = delete;
    MessageListener &operator=(const MessageListener &) = delete;

    inline MessageListener(MessageListener &&other) noexcept : m_impl(other.m_impl)
    {
