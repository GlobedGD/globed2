...
template <size_t N> static inline constexpr const char *constant(const char (&str)[N])
{
    return constantByHash(adler32(str));
}

template <ConstexprString str> static inline constexpr const char *constant()
{
