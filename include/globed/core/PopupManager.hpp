...
public:
    ...
    // Creates a popup with the given title and content (optionally button 1, 2 and width). Does not show the popup to
    // the user.
    PopupRef alert(CStr title, const std::string &content, CStr btn1 = "Ok", CStr btn2 = nullptr,
                   float width = DEFAULT_WIDTH);

    // Creates a popup with the given title and content (optionally button 1, 2 and width). Does not show the popup to
    // the user. The callback is involved when the user presses either of the buttons in the popup.
    PopupRef quickPopup(CStr title, const std::string &content, CStr btn1 = "Ok", CStr btn2 = nullptr,
    ...
    template <class... Args> PopupRef alertFormat(CStr title, fmt::format_string<Args...> fmt, Args &&...args)
    {
...
inline void alert(CStr title, const std::string &content, CStr btn1 = "Ok", CStr btn2 = nullptr,
...
inline void quickPopup(CStr title, const std::string &content, CStr btn1 = "Ok", CStr btn2 = nullptr,
                       std23::move_only_function<void(FLAlertLayer *, bool)> callback = {},
                       float width = PopupManager::DEFAULT_WIDTH)
{
    PopupManager::get().quickPopup(title, content, btn1, btn2, std::move(callback), width).showInstant();
}

/// Creates a confirmation popup and shows it to the user.
/// Shorthand for quickPopup(...), but with the callback having 1 argument and only being invoked when the user presses
/// the second button.
inline void confirmPopup(CStr title, const std::string &content, CStr btn1 = "Cancel", CStr btn2 = "Ok",
...
template <class... Args> void alertFormat(CStr title, fmt::format_string<Args...> fmt, Args &&...args)
{
...
template <class... Args> void toast(geode::NotificationIcon icon, fmt::format_string<Args...> fmt, Args &&...args)
{
...
template <class... Args> void toastError(fmt::format_string<Args...> fmt, Args &&...args)
{
...
template <class... Args> void toastSuccess(fmt::format_string<Args...> fmt, Args &&...args)
{
